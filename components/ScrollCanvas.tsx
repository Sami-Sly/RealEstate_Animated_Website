...
//     // FAULT TOLERANCE: If user scrolls fast and frame isn't loaded, find the closest previous loaded frame.
...
//       // OPTIMIZATION: Do not wait for 240 frames. Show site immediately after first 2 frames are ready.
//       if (loaded >= 2 && !contentShownRef.current) {
//         contentShownRef.current = true;
//         setShowContent(true);
//         setTimeout(() => setIsVisible(true), 50); // Fade in immediately
//       }
//     };
...
//     // OPTIMIZATION: Only preload the first 3 frames blocking to save main thread.
//     for (let i = 0; i < Math.min(3, TOTAL_FRAMES); i++) loadFrame(i);

//     // OPTIMIZATION: Load the rest completely silently in the background
//     const ric = (window as any).requestIdleCallback || ((cb: Function) => setTimeout(cb, 100));
//     ric(() => {
//       for (let i = 3; i < TOTAL_FRAMES; i++) {
//         setTimeout(() => loadFrame(i), i * 15); // Stagger requests slightly to prevent network choke
//       }
//     });
//   }, [drawFrame]);
...
//     padding: 0 6vw;
//   }
//   .sc-beat-inner { width: 100%; max-width: 420px; }

//   /* Data-Align Layout Controls */
...
//   .sc-beat[data-align="left"] .sc-pill-wrapper, 
//   .sc-beat[data-align="left"] .sc-rule-wrapper { justify-content: flex-start; }
  
//   .sc-beat[data-align="right"] .sc-pill-wrapper, 
//   .sc-beat[data-align="right"] .sc-rule-wrapper { justify-content: flex-end; }
...
//     font-size: clamp(32px, 4vw, 54px); margin-bottom: 16px;
//   }
//   .sc-beat[data-align="center"] .sc-hed { font-size: clamp(44px, 8vw, 100px); }

//   .sc-body-text {
//     font-family: 'DM Sans', sans-serif; font-weight: 300;
//     font-size: clamp(14px, 1.5vw, 16px); line-height: 1.72;
//     color: rgba(255,248,235,.52); letter-spacing: .01em;
//   }
...
//   /* Bullets */
//   .sc-bullets-wrapper { list-style: none; display: flex; flex-direction: column; gap: 10px; padding: 0; margin: 0; }
//   .sc-bullets-wrapper li {
//     font-family: 'DM Sans', sans-serif; font-weight: 300;
//     font-size: clamp(13px, 1.4vw, 15px); color: rgba(255,248,235,.52);
//     display: flex; align-items: center; gap: 10px; letter-spacing: .02em;
//   }
//   .sc-bullet-dot { width: 4px; height: 4px; border-radius: 50%; background: var(--gold); flex-shrink: 0; }
//   .sc-beat[data-align="right"] .sc-bullets-wrapper li { flex-direction: row-reverse; }
...
//   .sc-btn-ghost {
//     font-family: 'DM Sans', sans-serif; font-size: 11px;
//     letter-spacing: .08em; text-transform: uppercase; color: var(--text-dim); text-decoration: none;
//     border-bottom: 1px solid rgba(255,248,235,.12); padding-bottom: 2px; transition: color .2s, border-color .2s;
//   }
//   .sc-btn-ghost:hover { color: var(--text); border-color: rgba(255,248,235,.35); }
...
//     .sc-beat-inner { max-width: 350px; }
//   }

//   @media (max-width: 768px) {
//     /* Mobile: Push text toward bottom to keep image visible, center everything */
//     .sc-beat { 
//       padding: 0 24px; 
//       align-items: flex-end; 
//       padding-bottom: 25vh; 
//     }
//     .sc-beat-inner { max-width: 100%; text-align: center !important; }
    
//     .sc-beat[data-align="left"], 
//     .sc-beat[data-align="right"] { justify-content: center; }
    
//     .sc-pill-wrapper, 
//     .sc-rule-wrapper { justify-content: center !important; }
    
//     .sc-rule { display: none; } 
    
//     .sc-bullets-wrapper li { flex-direction: row !important; justify-content: center; }
//     .sc-final-cta { bottom: 6%; }
//   }
// `;
// // ─────────────────────────────────────────────────────────────────────────────
...
//     // FAULT TOLERANCE: Fallback to nearest loaded frame
//     while (i >= 0 && !loadedRef.current[i]) {
//       i--;
//     }
...
//     const onResize = () => {
//       // FIX 1: Only refresh canvas/GSAP if the window WIDTH changes. 
//       // This stops mobile scrollbar shrinking from breaking the scroll.
...
//       const a   = BEATS[i].align;
//       const mob = window.innerWidth < 768;
//       // FIX 2: Better alignment calculation based on device width
//       const ox  = mob ? 0 : a === "left" ? -22 : a === "right" ? 22 : 0;
//       const oy  = a === "center" ? 14 : 0;
...
//                   {/* RESTORED: Your exact original professional headline sizing */}
//                   <h2
//                     className="sc-hed"
//                     style={{
//                       fontSize:
...
//                   {/* RESTORED: Your exact original professional body text opacities & sizes */}
//                   {beat.body && (
//                     <p
//                       style={{
//                         fontFamily: "'DM Sans', sans-serif",
...
//                   {/* RESTORED: Your exact original bullet sizing */}
//                   {beat.bullets && (
//                     <ul className="sc-bullets-wrapper">
//                       {beat.bullets.map((b, bi) => (
//                         <li
//                           key={bi}
//                           style={{
