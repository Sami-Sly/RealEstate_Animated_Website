...
// ─────────────────────────────────────────────────────────────────────────────
// STORY BEATS (Update these for your Real Estate project)
// ─────────────────────────────────────────────────────────────────────────────
interface Beat {
  align: "center" | "left" | "right";
  eyebrow?: string;
...
const BEATS: Beat[] = [
  {
    align: "center",
    headline: "The Villa",
    body: "An architectural masterpiece in the hills.",
    sectionId: "story",
  },
  {
    align: "left",
    eyebrow: "Design",
    headline: "Seamless\nIndoor-Outdoor.",
    body: "Every room is designed to capture natural light and panoramic views of the skyline.",
    sectionId: "ingredients",
  },
  // ... Keep or update your other beats here
];
...
export default function ScrollCanvas() {
  ...
  const drawFrame = useCallback((idx: number) => {
    ...
    // ─────────────────────────────────────────────────────────────────
    // RESPONSIVE FIX: "Show the entire villa"
    // Portrait screens (Mobile) use 'contain' so sides aren't cut off.
    // Landscape screens (Desktop) use 'cover' for full bleed.
    // ─────────────────────────────────────────────────────────────────
    ...
    const dh = imgH * scale;
    const dx = (W - dw) / 2;
    const dy = (H - dh) / 2; // Centers the frame vertically and horizontally
  ...
  useEffect(() => {
    let isCancelled = false; // Fixes React StrictMode memory leak
    let loaded = 0;
    ...
    const loadFrame = (i: number): Promise<void> => {
      return new Promise((resolve) => {
        ...
        // Removed img.decode()! The browser will just hold the compressed file in RAM.
        img.onload = () => {
          if (isCancelled) return resolve();
          ...
          // Show site after 3 frames are ready
          if (loaded >= 3 && !contentShownRef.current) {
            contentShownRef.current = true;
            setShowContent(true);
            setTimeout(() => setIsVisible(true), 50); 
          }
    ...
    const preloadSequence = async () => {
      // 1. Load first 5 frames rapidly
      for (let i = 0; i < Math.min(5, TOTAL_FRAMES); i++) {
        await loadFrame(i);
      }
      
      // 2. Load the rest sequentially. This strictly prevents the 300 
      // simultaneous network requests that were freezing your PC.
    ...
    <>
      ...
      >
        <div className="sc-sticky">
          ...
          <div className="sc-final-cta">
            <a href="#acquire" className="sc-btn-gold">View Property</a>
            <a href="#details" className="sc-btn-ghost">View Floorplans</a>
          </div>
        </div>
      </div>
    </>
